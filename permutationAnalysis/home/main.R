...
clustering.output=read.table(paste("./",matrixName,"_clustering.output.",format,sep=""),header=TRUE,sep=separator2)
...
ScoreBCs=ScoreBCs+relationMatrix0 #OK QUINDI???
    permutationMatrix=read.table(paste("./",matrixName,"_final_clusterP.",format,sep=""),header=TRUE,sep=separator2)

   # permutationMatrix=permutationMatrix[,which(apply(permutationMatrix,2,max)==i)]

    killedMatrix=as.matrix(read.table(paste("./",matrixName,"_final_killedCell.",format,sep=""),header=TRUE,sep=separator2))
...
write.table(score,paste("./",matrixName,"_score.",format,sep=""),col.names=NA,sep=separator2)
finalScore=rowSums(score)/ncol(killedMatrix)
write.table(finalScore,paste("./",matrixName,"_scoreSum.",format,sep=""),col.names=FALSE,sep=separator2)
pdf(paste("./",matrixName,"_Stability_Plot.pdf",sep=""))
...
write.table(clustering.output,paste("./",matrixName,"_clustering.output.",format,sep=""),sep=separator2,row.names=FALSE)
pdf(paste("./",matrixName,"_CCPlot.pdf",sep=""))
...
write.table(labelComponent,paste("./",matrixName,"_labelComponent.",format,sep=""),col.names=NA,sep=separator2,row.names=TRUE)
 for(i in 1:nCluster){
...
pdf(paste("./",nCluster,"_Stability_Violin_Plot.pdf",sep=""))
